...
def sort_dicts_by_date(data, descending=True):
    """
    Функция сортирует список словарей по дате (ключ 'date'), может принимать необязательный аргумент для задания порядка сортировки (по убыванию или возрастанию).

    :data: Список словарей, который нужно отсортировать.
    :descending: Логический флаг, указывающий на то, что сортировка должна быть выполнена по убыванию (True) или по возрастанию (False).
